...
def insert_data(name, res_score, timestamp, company, college, reco_field, cand_level, skills, recommended_skills,
                courses):
    DB_table_name = 'user_data'
    insert_sql = "insert into " + DB_table_name + """
    values (0,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
    rec_values = (
    name, str(res_score), timestamp, company, college, reco_field, cand_level, skills, recommended_skills,
    courses)
...
def run():
    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            ...
            #working part
            resume_data = fitz.open(save_image_path)
            text = " "
            for page in resume_data:
                text = text + str(page.get_text())
            text = text.strip()
            text = ' '.join(text.split())
            resume_data = nlp(text)

            resume_data = {ent.label_: ent.text for ent in resume_data.ents}
            ...
            if resume_data:
                ...
                ## Skill shows
                pattern = r'[,•]'
                resumeSkills = re.split(pattern, resume_data['Skills'])
                keywords = st_tags(label='### Skills that you have',
                                   text='See our skills recommendation',
                                   value=resumeSkills, key='1')
                print(resume_data["Skills"], "FAGGOT")
                ...
                insert_data(resume_data['Name'], str(resume_score), timestamp, str(resume_data['Companies worked at']),
                            str(resume_data['College Name']),
